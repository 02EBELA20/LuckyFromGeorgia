...
async function handler(req, res) {
    ...
    try {
        ...
        const vipWinner = await winnersCollection.findOne({
            isVip: true
        });
        client.close();
        if (!vipWinner) {
            return res.status(404).json({
                message: "No VIP winner found"
            });
        }
        res.status(200).json(vipWinner);
    } catch (error) {
        res.status(500).json({
...
//# sourceMappingURL=%5Broot%20of%20the%20server%5D__1fc70d._.js.map